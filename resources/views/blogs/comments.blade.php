...
<body>
    <form method="POST" action="/blogs/{{$blog->id}}">
        ...
        <div class="flex items-center">
            ...
            @endauth
            <div class="flex-1">
                <textarea name="content" class="w-full bg-gray-100 rounded p-2 focus:outline-none" rows="3" placeholder="Add a comment..."></textarea>
                    ...
                    @auth
                        ...
                        <div id="edit-comment-{{ $comment->id }}" class="mt-4" style="display: none;">
                            <form action="{{ route('comments.update', ['blog' => $blog->id, 'comment' => $comment->id]) }}" method="POST" class="bg-gray-100 p-4 rounded">
                                @csrf
                                @method('PUT')
                                <textarea name="content" rows="4" class="w-full p-2 bg-white border border-gray-200 focus:outline-none" placeholder="">{{ $comment->content }}</textarea>
